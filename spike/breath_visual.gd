...
var _phase: BreathModel.Phase = BreathModel.Phase.INHALE
...
func _on_breath_tick(phase: BreathModel.Phase, _phase_progress: float, amplitude: float, _cycle_time: float) -> void:
	_amplitude = amplitude
	_phase = phase
	queue_redraw()

func _draw() -> void:
	var radius: float = lerpf(min_radius, max_radius, _amplitude)
...
func _phase_color(phase: BreathModel.Phase) -> Color:
	match phase:
